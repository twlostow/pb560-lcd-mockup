...
// Registre de controle du timer T4
	// Pr�diviseur = 1024 T4I = 111  
	// Mode timer 	   T4M = 000
	// Timer en marche T4R = 1
	// Direction "d�comptage" sans tenir compte de l'entr�e T4 T4EUD = 01
	// Contr�le de la sortie T4OUT P31 	non utilis�e.T6OE = 0
	// latch de la sortie T4OL = 0
	// Bit 9 10 11 12  13 14 15 non utilis�s = 0000000
	// Controle du mode reload pas de reload � partir de CAPREL	 T4SR = 0
	// T4CON =  00000 0 0 01 1 000 000 = 0000000011000111 b = 0xC7
T4CON = 0xC7;

// Registre de contr�le de l'interruption
	// Revoir sa localisation dans le projet voir peut etre pour un 
	// module de gestion des niveaux de priorit� des interruptions 
	// et de gestion des PEC
		// Priorit�  des interruptions T4
...
/*%C ACC1,ACC2,CCMOM1,CCMOD2 non programm�s			 */
/*%C ==> CCM0 = xxxx xxxx xxxx 1001 = 0x0009			 */
	...
	/*%C	Init interruption XIR0SEL (bit7)	 */
		...
		// P8.6 Input 
		XS1PORT = 0x18; 
		// registre d'interruption sur �mission 
		/*%IR = 0 ,IE = 0; ILVL  = 13 ,GLVL = 1 	 */
		XP0IC = 0x35; 



		// registre de configuration de la liaison s�rie SS0
			// S0M asynchrone  8bits sans parit�  = 001
			// S0STP 1 bit de stop = 0
			// S0REN R�ception autoris�e = 1
			// S0PEN Test de parit� en reception  non autoris�e = 0
			// S0FEN Test de reception du stop non autris� = 0
			// S0OEN Test d'ecrassement des data en r�ception  non autoris� = 0
			// S0PE  Flag d'erreur de parit� = 0
			// S0FE 	Flag d'erreur sur stop	 = 0
			// S0OE  Flag d'�crasemement = 0
			// S0ODD parit� paire = 0
			// S0BRS mode de calcul du baud rate  = 0
			// S0LB Loop back mode non autoris� = 0
			// S0R g�n�rateur de baud rate activ� = 1
			// ==> S0CON = 100000000000001  = 0x8011;
		//XS1CON =  0x8011;
		//Activation de l'interruption de reception sur XP0INT
		//XIR0SEL |= 0x8000;
		// registre de g�n�rateur de baudrate = 0x81 pour 9600 baud
...
/*%C unactivation SPI		  */
	SSCCON = 0x0000;
// pr�positionement des niveaux des lignes SCLK et TSR  sur le bus SPI 	  
	SPI_PORT |= 0x2200;    
	DRV_OutputPort(CE_RTC1, 0); /*%C Desactivation chip enable RTC																*/
	DRV_OutputPort(CE_EEP, 1); /*%C Desactivation chip enable EEPROM */
	SSCTIC = 0x0000;     /*%C interruption unactive														*/
...
/********************************************************************************/
	...
	// V�rifier qu'� cet instant tous les drivers sont initialis�s
	// Init Interrupt6 - 1ms
	//	InitInterruptT6();
	// Registre de controle du timer T6
		// Pr�diviseur = 4 T6I = 000 horloge � 100ns pour un ST269 � 40Mhz
		// Mode timer 	   T6M = 000
		// Timer en marche T6R = 1
		// Direction "d�comptage" sans tenir compte de l'entr�e T6 T6EUD = 001
		// Contr�le de la sortie T6OUT P31 	non utilis�e.T6OE = 0
		// latch de la sortie T6OL = 0
		// Bit 11 12  13 non utilis�s = 000
		// Controle du mode reload Reload � partir de CAPREL	 T6SR = 1
...
/******************************************************************************/
	...
	// Initialisation du driver Buzzer (la base doit �tre initialis�e)
	DRV_BuzInit();	
...
{
// Initialisation de la com RS232 apr�s la lecture de la base pour 
//disponibilit� de SPEEDCOM
...
// registre de configuration de la liaison s�rie SS0
	// S0M asynchrone  8bits + parit�  = 111
	// S0STP 1 bit de stop = 0
	// S0REN R�ception autoris�e = 1
	// S0PEN Test de parit� en reception  non autoris�e = 0
	// S0FEN Test de reception du stop non autris� = 0
